...
/**
   ...
   VolumeRendererJ3D, within a DataDisplayLink, under Java3D.<P>
*/
public class ShadowVolByRefFunctionTypeJ3D extends ShadowFunctionTypeJ3D {
  ...
  public ShadowVolByRefFunctionTypeJ3D(MathType t, DataDisplayLink link, ShadowType parent) 
  ...
  */
  ...
  private void applyTexture(Shape3D shape, VisADImageTile tile, boolean apply_alpha, float constant_alpha) {
    ...
    */
	...
	/*private void initRegenFlags(VolumeRendererJ3D imgRenderer, ShadowFunctionOrSetType MyAdaptedShadowType,
  ...
  public boolean doTransform(Object group, Data data, float[] value_array,
    	...
    	if (data.isMissing()) {
      		((VolumeRendererJ3D) renderer).markMissingVisADBranch();
      		return false;
    	}
	...
	if (!adaptedShadowType.getIsTerminal()) {
	 ...
	 // check that range is single RealType mapped to RGB only
    	...
    	int color_length;
    	VolumeRendererJ3D imgRenderer = (VolumeRendererJ3D) renderer;
	...
	regen_colbytes = false;
  	regen_geom = false;
  	apply_alpha = false; 
	initRegenFlags((VolumeRendererJ3D)renderer, adaptedShadowType, constant_alpha, cmap, cmaps, data, display, default_values, value_array, valueToScalar, valueArrayLength, link, curved_size, hasAlpha);
	if(!reuseImages) {
		regen_geom = true;
		regen_colbytes = true;
		apply_alpha = true;
	...
	//REUSE GEOMETRY/COLORBYTE LOGIC (ENDS HERE)
     	prevImgNode = ((VolumeRendererJ3D)renderer).getImageNode();
	...
	*/
	if (!reuseImages || (regen_colbytes && regen_geom)) { //REUSE GEOM/COLBYTE:Earlier reuse variable was used. Replaced it with reuseImages.
		...
		BranchGroup branch = new BranchGroup();
       		...
       		((VolumeRendererJ3D)renderer).setImageNode(imgNode);
         ...
         if (group instanceof BranchGroup) {
           ((VolumeRendererJ3D) renderer).setBranchEarly((BranchGroup) group);
         }
         */
       		}
       		...
       		imgNode = ((VolumeRendererJ3D)renderer).getImageNode();
	...
	Mosaic mosaic = new Mosaic(data_height, texture_height_max, data_width, texture_width_max);
        ...
        for (int k=0; k<data_depth; k++) {
           new VisADImagePlane(numImages, data_height, 0, data_width, 0, 1, 0);
        }
        // perp to axis 1
        for (int k=0; k<data_height; k++) {
           new VisADImagePlane(numImages, 1, 0, data_width, 0, data_depth, 0);
        }
        // perp to axis 2
        for (int k=0; k<data_width; k++) {
           new VisADImagePlane(numImages, data_height, 0, 1, 0, data_depth, 0);
        }
